...
To start it, please start the database and run the migrations
...
At the moment, the VTN brings their own OAuth service,
...
but the code base is prepared to allow other data stores as well in the future.
Again, we warmly welcome contributions or sponsoring if you are interested in adding support for another storage.

The VEN is a library for conveniently interact with the REST API provided by a VTN.
...
We prefer to know when you intend to develop some functionality to make sure that there aren't multiple people working on the same issue.
Simple drop a short note to the corresponding issue.
Please make sure that you add a `signed-off-by` appendix to your commit message,
...
By that, you acknowledge the text in [`CONTRIBUTING`](CONTRIBUTING).
The easiest way is to add a `-s` flag to the `git commit` command, i.e., `git commit -s`.
...
If you cannot contribute yourself but are happily using this code and would like it to develop further,
we are also happy to accept sponsoring of any amount.
Please [reach out to Hugo](mailto:hugo@tweedegolf.com) if you are interested. 
